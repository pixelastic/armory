...
module.exports = {
  ...
  coordinates(record) {
    ...
    const lat = this.coordinate(hash.slice(0, 6));
    const lng = this.coordinate(hash.slice(6, 12));
    return { lat, lng };
  },
  coordinate(input) {
    // Take 5 first chars, convert them to integers, add them
    // Gives a number between 5 and 80
    const value = _.chain(input)
      .slice(0, 5)
      .map((char) => {
        return _.parseInt(char, 16);
      })
      .sum()
      .value();
    // Check for sign based on if last number is odd or even
    return _.parseInt(input[5], 16) % 2 ? value : -value;
  },
};
