...
import { _, firost, chalk, pMap } from 'golgoth';
    ...
    let records = await pMap(
      ...
      async page => {
        ...
        if (await wiki.isRedirect(title)) {
          spinner.tick(chalk.red(`${title} (redirect)`));
          return null;
        }

        // Skip sets and comparisons
        if (this.isSet(title) || this.isComparison(title)) {
          spinner.tick(chalk.red(`${title} (set or comparison)`));
          return null;
        }

        // Skip listings
        if (await this.isListingPage(title)) {
          spinner.tick(chalk.red(`${title} (Listing)`));
          return null;
        }

        // Skip items removed from released game
        if (await this.isRemoved(title)) {
          spinner.tick(chalk.red(`${title} (removed from game)`));
          return null;
        }

        // Skip items from beta
        if (await this.isBeta(title)) {
          spinner.tick(chalk.red(`${title} (beta content)`));
          return null;
        }

        // Skip achievements
        if (await this.isAchievement(title)) {
          spinner.tick(chalk.red(`${title} (achievement)`));
          return null;
        }
        ...
        if (!description) {
          spinner.tick(chalk.red(`${title} (no description)`));
          return null;
        }
        ...
        // Skip empty pictures
        if (!picture) {
          spinner.tick(chalk.red(`${title} (no pictures)`));
          return null;
        }
